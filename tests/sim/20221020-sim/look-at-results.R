...
simDir <- "~/Github/Dvar/tests/sim/20221008-sim"
nsim <- 10
...
# Loop over each results##.RData file and store results
for(jj in seq_along(simLevels)){
  if(jj==1) next # skip the first Workload (filled manually after loop)
...
save(R, X, WLlist_fullLen, noise, A, epsMod, timeRunMat,
     file = "20221008-forEric-simdate20221008.RData")
